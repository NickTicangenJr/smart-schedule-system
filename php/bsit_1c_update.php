...
if(isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM bsit_1c WHERE id=$id";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
    }else{
        header("Location: bsit_1c.php");
    }
    ...
    include '../db_conn.php';
        ...
        // --    (monday_stat, tue_stat, wed_stat, thu_stat, fri_stat) 
           ...
           $result = mysqli_query($conn, $sql);
           if ($result) {
                 header("Location: ../bsit_1c.php?success=successfully updated");
           }else {
              header("Location: ../bsit_1c_update.php?error=unknown error occurred&$user_data");
           }
}
